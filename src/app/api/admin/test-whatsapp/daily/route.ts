...
export async function POST(req: Request) {
  try {
    ...
    if (!digest || digest.itemIds.length === 0) {
      return NextResponse.json(
        { error: "No matching or undelivered intelligence items available.", hint: "Reset delivery logs or process new articles." },
        { status: 404 }
      );
    }
    ...
    try {
      const messageId = await sendDigest(user.id, phone, "Daily Digest", digest.itemIds.length, digest.text.substring(0, 150));
      await prisma.deliveryLog.update({ where: { id: log.id }, data: { status: DeliveryStatus.SENT, sentAt: new Date(), messageId } });
      return NextResponse.json({ success: true, mode: "daily", deliveryLogId: log.id, messageId, itemsSentCount: digest.itemIds.length, template: "pulse_digest_ready" });
    ...
    console.error("Admin daily test delivery failed:", error);
